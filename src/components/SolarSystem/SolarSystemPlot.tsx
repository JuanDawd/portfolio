...
export function SolarSystemPlot({ width = 700, height = 700 }) {
  ...
  const initializePlanetPositions = (): PlanetPosition[] => {
    ...
    // Create a map of speeds based on orbit radius
    const speedMap = {
      100: 0.5, // Frontend Frameworks
      150: 0.4, // Styling Solutions
      200: 0.3, // Languages
      250: 0.2, // Design & Mobile
    }

    // Group planets by their orbit radius
    ...
    // Calculate positions for each orbit group
    Object.entries(planetsByRadius).forEach(([radius, planetsInOrbit]) => {
      ...
      planetsInOrbit.forEach((planet, index) => {
        ...
        positions.push({
          ...
          icon: planet.icon,
          angle: angle,
          speed: speedMap[orbitRadius as keyof typeof speedMap],
        })
      })
    })
